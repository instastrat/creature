    ...
    queryly = {
        ...
        init: function (querylykey, maincontent, config) {
            ...
            try {
                ...
                if ((typeof queryly.config != 'undefined' && queryly.config.pageservice) || queryly.querylydemo == 'related' || queryly.querylydemo == 'native' || queryly.querylydemo == 'hyperlink') {
                    try {
            ...
            try {
                ...
                if (typeof results.items == 'undefined' || results.items.length == 0) {
                    ...
                    if (queryly.language == 1) {
                        message = 'Es wurden keine Ergebnisse gefunden. Bitte versuche es mit einem anderen Stichwort.';
                    }
                    else if (queryly.language == 2) {
                        message = 'Aucun résultat trouvé. Veuillez rechercher un mot-clé différent.';
                    }
                    ...
                    queryly.searchcontainer.innerHTML = '<div style="margin: 50px;text-align: center;font-size: 30px;color: #444;">' + message + '</div>';
                    queryly.searchoutercontainer.style['display'] = 'block';
        ...
        drawLinks : function (data) {
            for (var i = 0; i < data.keywords.length; i++) {
                ...
                try {
                    ...
                    var rgx = new RegExp('\\b(' + words.join('|') + ')\\b(?![^<]*>|[^<>]*</(a|b|strong))', 'gi');
